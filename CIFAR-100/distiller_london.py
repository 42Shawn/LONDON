    ...
    def forward(self, x):
        ...
        for i in range(feat_num):
            ...
            if i < feat_num - 1:
                ...
                TM_s = torch.bmm(self.transmitting_matrix(s_feats[i], s_feats[i + 1]), self.transmitting_matrix(s_feats[i], s_feats[i + 1]).transpose(2,1)) #for calculate the eigenvalue of transmitting matrix, because it is asymmetric
                ...
                loss_london += F.mse_loss(self.top_eigenvalue(K=TM_s), self.top_eigenvalue(K=TM_t)) / 2 ** (feat_num - i - 1) #the relation near the front layer is more valuable

        return s_out, loss_distill, loss_london
