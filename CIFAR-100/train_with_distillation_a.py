#experiment on WRN16-4
#change the alpha choose strategy
...
parser.add_argument('--alpha', default=0.8, type=float, help='alpha in loss function')
parser.add_argument('--beta', default=1, type=float, help='alpha in loss function')
    ...
    with tqdm(total=len(trainloader)) as t:
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if args.is_distill_loss_applied and args.is_spectral_loss_applied:
                loss = loss_CE + args.beta*loss_distill.sum() / batch_size / 1000  + alpha*loss_spectral.sum() / 200
                # print('loss mode:',1)
                ...
                loss = loss_CE + alpha * loss_spectral.sum() / 200
                # print('loss mode:',3)
