...
训练WideResNet作为teacher net，paper表示其性能能达到80% acc
use WRN28-4
'''
import argparse
import logging
...
parser.add_argument('--paper_setting', default='a2', type=str)
...
parser.add_argument('--num_epochs', default = 200, type=int, help='Input the number of epoches: default(300)')
...
parser.add_argument('--schedule', type=int, nargs='+', default=[60, 120, 160, 200],
                    help='Decrease learning rate at these epochs.')
parser.add_argument('--efficient', action='store_true',
                    help='Decide whether or not to use efficient implementation(only of densenet): default(False)')
parser.add_argument('--weight_decay', default=5e-4, type=float, help='Input the weight decay rate: default(5e-4)')
...
parser.add_argument('--version', default='WRN16-4V0-pretrainedV1', type=str, help='Input the version of current model: default(V0)')
...
def train(train_loader, model, optimizer, criterion, accuracy, args):
    ...
    with tqdm(total=len(train_loader)) as t:
        for _, (train_batch, labels_batch) in enumerate(train_loader):
            ...
            # writer_graph.add_graph(model=model,input_to_model=train_batch)
...
def train_and_evaluate(model, train_loader, test_loader, optimizer, criterion, accuracy, model_dir, args):
    ...
    scheduler = MultiStepLR(optimizer, milestones=args.schedule, gamma=0.2)
    # scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.2, verbose=True, patience=10)
    # scheduler = CosineAnnealingLR(optimizer, T_max= 20, last_epoch=-1)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if not os.path.isdir('/home/user/shang/prg_kd/overhaul-distillation-master'):
            os.mkdir('/home/user/shang/prg_kd/overhaul-distillation-master')
        torch.save(state, '/home/user/shang/prg_kd/overhaul-distillation-master' + args.version+ '_WRN16-4_21.pt')
...
if __name__ == '__main__':
    ...
    model_dir = os.path.join('/home/user/shang/prg_kd/overhaul-distillation-master', args.dataset, str(args.num_epochs), args.paper_setting + args.version)
