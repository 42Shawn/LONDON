...
parser.add_argument('--paper_setting', default='a2-3', type=str)
parser.add_argument('--epochs', default=200, type=int, help='number of total epochs to run')
parser.add_argument('--batch_size', default=64*2, type=int, help='mini-batch size (default: 256)')
parser.add_argument('--lr', default=0.1, type=float, help='initial learning rate')
parser.add_argument('--alpha', default=1, type=float, help='alpha in loss function')
    ...
    with tqdm(total=len(trainloader)) as t:
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if args.is_distill_loss_applied and args.is_spectral_loss_applied:
                loss = loss_CE + loss_distill.sum() / batch_size / 1000  + alpha*loss_spectral.sum() / 200
                # print('loss mode:',1)
