#Teacher:resnet152 21.69 5.95, student resnet50 42.5% of teacher's net. OFD 21.75 5.83.
#alpha = 2000, bs = 96 Best accuracy (top-1 and 5 error): 21.861999244766235 5.705999829025268
...
import distiller_london as distiller

import warnings
warnings.filterwarnings("ignore")

parser = argparse.ArgumentParser(description='PyTorch ImageNet-1k Training')
parser.add_argument('--data_path', type=str, help='/home/user/shang/data/imagenet/')
parser.add_argument('--net_type', default='mobilenet', type=str, help='networktype: resnet, mobilenet')
parser.add_argument('-j', '--workers', default=10, type=int, help='number of data loading workers (default: 4)')
parser.add_argument('--epochs', default=100, type=int, help='number of total epochs to run')
parser.add_argument('--alpha', default=2000, type=float, help='alpha of spectral loss function')
parser.add_argument('-b', '--batch_size', default=96, type=int, help='mini-batch size (default: 256)')
parser.add_argument('--lr', '--learning_rate', default=0.1, type=float, help='initial learning rate')
parser.add_argument('--momentum', default=0.9, type=float, help='momentum')
parser.add_argument('--weight_decay', default=1e-4, type=float, help='weight decay (default: 1e-4)')
parser.add_argument('--print_freq', default=100, type=int, help='print frequency (default: 500)')
...
def main():
    ...
    traindir = '/home/user/shang/data/imagenet/train'
    # valdir = os.path.join(args.data_path, 'val')
    valdir = '/home/user/shang/data/imagenet/val'
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])

    # datasets.ImageNet(traindir,split='train')
    # datasets.ImageNet(traindir)
...
def train_with_distill(train_loader, d_net, optimizer, criterion_CE, epoch):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        loss = loss_CE + loss_distill.sum() / batch_size / 10000 + args.alpha * loss_spectral.sum() / batch_size / 10000
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    directory = "home/user/shang/prg_kd/%s/"%(args.net_type)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'home/user/shang/prg_kd/%s/'%(args.net_type) + 'model_best.pth.tar')
